...
\bottomrule
\end{tabular*}
\caption{Comparison of the probing models on the OntoNotes test set with various components removed. The head-finding attention and span-width embeddings contribute significantly to the performance of the probing model.}
...
\bottomrule
\end{tabular*}
\caption{Confusion Matrix for \textit{BERT-base c2f}. Subset size is 1250 examples.}
...
\bottomrule
\end{tabular*}
\caption{Confusion Matrix for \textit{BERT-large c2f}. Subset size is 1250 examples.}
\label{table:confusion-matrix-large}
\end{table}

Moreover, the errors were also categorised into \textit{true positives, true negatives, false positives,} and \textit{false negatives}. The numbers are provided in Table \ref{table:confusion-matrix-base} and \ref{table:confusion-matrix-large}. We observe that false positives constitute 4.08\% of the predictions for \textit{BERT-base c2f}, while false negatives make up 3.36\% of predictions of  \textit{BERT-base c2f}. \textit{BERT-large c2f} yields only 3.2\% false positives, but a slightly higher 3.5\% false negatives.
...
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l *{4}{d{1.2}} }
\toprule
\multicolumn{1}{c}{Category} & \multicolumn{1}{c}{Snippet} & \multicolumn{1}{c}{BERT-base c2f} & \multicolumn{1}{c}{BERT-large c2f}  \\
\midrule
\midrule
...
\midrule
Gender & \multicolumn{1}{l}{\makecell[l]{-}} & 0 & 0 \\
\midrule
...
\bottomrule
\end{tabular*}
\caption{Error Analysis of \textit{BERT-base c2f} and \textit{BERT-large c2f} models for examples with middle-range coreference (26-100 tokens apart). False positives are denoted \textbf{bold}, false negatives -- \textit{italic}. }
