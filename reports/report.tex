...
\section{Overview of BERT-coref}
...
Span representation plays an important role in span-ranking model [Lee 2017], since it is used to compute distribution over candidate antecedent spans. In order to be able to predict coreference relations accurately, span representation should also capture the information of the span's internal structure and its surrounding context. In our experiments, we use the span representation which is first proposed in [Lee 2017], but with BERT [Devlin] instead of a LSTM-based encoder to encode lexical information of the span and its surrounding, following [Joshi 2019]. The span representation is a vector embeddings which consists of context-dependent boundary representations with attentional representation of the head words over the span. The boundary representations are composed of first and last word-pieces of the span itself. The head words are automatically learned using additive attention [Bahdanau 2015] over each word-pieces in the span:
...
%To perform coreference resolution, it is important to choose a span representation so that the information of the internal structure of the mention span and its surrounding context can be accurately captured. We use the span representation which is first proposed in (End-to-End), but with BERT as encoder (Joshi) instead of a LSTM-based encoder. For span $i$, the span representation $g_{i}$ is formulated as the following:
...
We focus on coreference arc prediction task, which is a part of probing tasks suite for contextual word embeddings [Liu 2019, Tenney 2019]. In this task, a probing model is trained to determine whether if two mentions refer to the same entity. We use the OntoNotes dataset [Pradhan et al 2012] from the CoNLL-2012 shared task. As OntoNotes only contains annotations for corefering mentions, we produce negative samples following the approach by [Liu 2019]......

We also follow the approach of [Tenney 2019] by using spans of word-pieces, as [Liu 2019]'s approach is limited on single-token entities and therefore unable to fully exploit the whole mention-span.
